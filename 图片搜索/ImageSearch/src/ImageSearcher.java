...
public class ImageSearcher {
	...
	public TopDocs searchQuery(String queryString,String field,int maxnum){
		try {
			...
			while (ts.incrementToken()) {
				Term term = new Term(field, cta.toString());
				query.add(new SimpleQuery(term, avgLength), BooleanClause.Occur.MUST);
			}
			query.setBoost(1.0f);
	...
	public static void main(String[] args){
		...
		TopDocs results=search.searchQuery("宋祖德", "abstract", 100);

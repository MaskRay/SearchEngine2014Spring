...
final class SimpleScorer extends Scorer {
	...
	SimpleScorer(Weight weight, TermDocs td, Similarity similarity,
			byte[] norms, float idfValue, float avg, IndexSearcher searcher) {
	...
	public float score() {
		assert doc != -1;
		//DONE: implements BM25 ranking algorithm
		//return idf * this.termDocs.freq();
		try {
			Document document = searcher.doc(doc);
			float len = document.getField("abstract").stringValue().length(), tf = freqs[pointer];
			return (K1+1)*tf/(K1*(1+b*(len/avgLength-1))+tf)*idf;
		} catch (Exception e) {
			e.printStackTrace();
			return idf * this.termDocs.freq();
		}
